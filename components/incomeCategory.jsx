...
import { incomeCategories } from '../utils/category';
...
export default function IncomeBreakdown() {
  ...
  const categorizedData = transactions.map((tx) => {
    const category = incomeCategories.find((c) => c.name === tx.category);
    return {
      ...tx,
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity onPress={() => router.push(`/category/${encodeURIComponent(item.category)}`)}
  style={{
    backgroundColor: '#fff',
    padding: 10,
    marginBottom: 12,
